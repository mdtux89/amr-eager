    ...
    def triples2String(self, triples):
	    ...
	    #print(triples[i])
            ...
            if c2 == "" and v2 != "TOP":
                ...
                usedrightafter = ((i < len(triples) - 1) and triples[i + 1][0] == v2)
                noothermentions = (len([t for t in triples[i+1:] if t[0] == v2 or t[3] == v2]) == 0)
                # print(v2)
                # print([t for t in triples[i+1:] if t[0] == v2 or t[3] == v2])
                # print(usedrightafter, noothermentions)
                if v2 not in old_instances and (noothermentions or usedrightafter):
                    instance_triple = (variables[v2],':instance-of', concepts[c2])
