...
function loadDataset(dataFileTrain, dataFileValid, nclasses, wordsDataLength)
  ...
  local trainInput = dp.ListView({dp.DataView('bf', xa)})
  ...
  local validInput = dp.ListView({dp.DataView('bf', xa)})
...
function loadExperiment(opt, dictSizeWords, outputSize, wordsDataLength)

        inputSize = (wordsDataLength * opt.inputEmbeddingSizeWords)
        local inputs = {}
...
--4, 68, 12, 4, 6
dataset = loadDataset(train, valid, 2, 3)
cmd = torch.CmdLine()
cmd:text()
cmd:text('Options:')
...
xp = loadExperiment(opt, 149507, 2, 3) --106, 21, 6, 10
xp:run(dataset)
