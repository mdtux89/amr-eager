...
function loadDataset(dataFileTrain, dataFileValid, nclasses, wordsDataLength, posDataLength)
  ...
  local xa = torch.Tensor(dataSize, wordsDataLength)
  local xb = torch.Tensor(dataSize, posDataLength)
  ...
  for line in io.lines(dataFileTrain) do
      ...
      xa[i] = inp[{{1, posDataOffset - 1}}]
      xb[i] = inp[{{posDataOffset, nFeats}}]
	    i = i + 1
  end

  local trainInput = dp.ListView({dp.DataView('bf', xa), dp.DataView('bf', xb)})
  ...
  local validInput = dp.ListView({dp.DataView('bf', xa), dp.DataView('bf', xb)})
...
function loadExperiment(opt, dictSizeWords, dictSizePos, outputSize, wordsDataLength, posDataLength)

        inputSize = (wordsDataLength * opt.inputEmbeddingSizeWords) + (posDataLength * opt.inputEmbeddingSizePos)
        local inputs = {}
        ...
        paralltab:add(words)
        paralltab:add(pos)
        embs = paralltab({inputs[1],inputs[2]})
...
local nGl = 0
for line in io.lines("resources/graphlets.txt") do
    nGl = nGl + 1
end

train = "/disk/scratch/s1333293/t2s_dataset_train.txt"
valid = "/disk/scratch/s1333293/t2s_dataset_valid.txt"
--4, 68, 12, 4, 6
dataset = loadDataset(train, valid, nGl, 4, 4)
cmd = torch.CmdLine()
cmd:text()
cmd:text('Options:')
...
xp = loadExperiment(opt, 149507, 52, nGl, 4, 4) --106, 21, 6, 10
xp:run(dataset)
