...
for amr_pred, amr_gold in zip(pred, gold):
	...
	vardict1 = amr1.var2concept()
	amr2 = src.amr.AMR(amr_gold)
	vardict2 = amr2.var2concept()

	list_pred = [str(vardict1[v1])+str(v2) for (v1,l,v2) in amr1.role_triples() if l == ":name"]
	list_gold = [str(vardict2[v1])+str(v2) for (v1,l,v2) in amr2.role_triples() if l == ":name"]
